#' Unmixing WLS, standard solution for a list of samples
#'
...
#' @export
unmix_WLS_list <- function(M,fs_raw_dir, aurora_channel_change = FALSE, channels = NULL){
...
# load samples
  ...
  unmixed_sample_list <- lapply(fs_raw, FUN = function(fs){
    ...
    # WLS unmixing solution
    unmixed_fluo <- t(wmtm_1 %*% M_t %*% W %*% sample_fluo)
